...
<style>
    ...
    /* Selectbox en sidebar - CORREGIDO */
...
# ⭐ APLICAR AJUSTE DE PASAJEROS ⭐
original_passenger_count = int(df_flight.iloc[0]['Passenger_Count'])

if adjusted_passenger_count != original_passenger_count:
    # Modificar el DataFrame con el nuevo número de pasajeros
    df_flight['Passenger_Count'] = adjusted_passenger_count
    
    # Recalcular Qty_Per_Passenger con el nuevo valor
    df_flight['Qty_Per_Passenger'] = (
        df_flight['Standard_Specification_Qty'].fillna(0)
        .div(df_flight['Passenger_Count'].replace(0, np.nan))
        .fillna(0)
    )
...
with col4:
    # Mostrar pasajeros con indicador si fue modificado
    passenger_display = int(row0.get('Passenger_Count', 0))
    is_modified = adjusted_passenger_count != original_passenger_count
    
    gradient = "linear-gradient(135deg, #FFD700 0%, #FFA500 100%)" if is_modified else "linear-gradient(135deg, #43e97b 0%, #38f9d7 100%)"
    
    st.markdown(f"""
    <div style="text-align: center; padding: 1rem; background: {gradient}; 
                border-radius: 10px; color: white;">
        <div style="font-size: 2rem;">👥</div>
